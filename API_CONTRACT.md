...
| 6 | [`/api/v1/facility/beds`](#6-post-apiv1facilitybeds) | `POST` | PHC Beds Update |
| 7 | [`/api/v1/facility/attendance`](#7-post-apiv1facilityattendance) | `POST` | PHC Attendance Update |
...
## 6. `POST /api/v1/facility/beds`

Logs an **available beds update** for a Primary Health Centre.
...
| Field | Type | Required | Notes |
|---|---|---|---|
| `phc_id` | `string` | ✅ | Primary Health Centre identifier |
| `available_beds` | `integer` | ✅ | Number of available beds — must be ≥ 0 |
...
{
  "status": "success",
  "message": "Beds update logged"
}
```

---

## 7. `POST /api/v1/facility/attendance`

Logs a **doctor attendance update** for a Primary Health Centre.
...
|---|---|---|---|
| `phc_id` | `string` | ✅ | Primary Health Centre identifier |
| `doctors_present` | `integer` | ✅ | Number of doctors present — must be ≥ 0 |
| `timestamp` | `string` | ✅ | ISO 8601 datetime |
...
{
  "status": "success",
  "message": "Attendance update logged"
}
```
...
POST  /api/v1/facility/beds                → Log a PHC beds update
POST  /api/v1/facility/attendance          → Log a PHC attendance update
```
